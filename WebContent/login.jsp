...
if(request.getParameter("btnlogin")!=null) {
    ...
    } else if(user.equals("Collector")) {
        ResultSet rs = db.getRows("select * from collectorlogin where EmailID=? and Password=?", mail, pass);
        if(rs.next()) {
            session.setAttribute("user", user);
            session.setAttribute("mail", mail);
            session.setAttribute("pass", pass);
            session.setAttribute("type", "collector");
            response.sendRedirect("/Dairy_management_system/collector/collector.jsp");
        } else {
            session.setAttribute("user", null);
            session.setAttribute("mail", null);
            session.setAttribute("pass", null);
